...
<body>
    ...
    <nav class="navbar">
        <div class="row" style="padding-top: 0px; padding-left:110px">
		 ...
		 <option value="" selected>All</option>
                <option value="7:00:00-11:59:59">Breakfast</option>
                    <option value="12:00:00-17:59:59" >Lunch</option>
                    <option value="18:00:00-23:59:59">Dinner</option>
...
<a style="background: #ec9a29; color:white;" href="javascript:void(0);" class="btn" data-toggle="modal" data-target="#Modal_Add"><span class="fa fa-plus"></span> Add New</a>
<a style="background: #ec9a29;" href="#" id="takeaway" class="btn btn-success" ><span class="fa fa-plus"></span> Takeaway</a>
</div>
                        </h1>
                    </div>
                </div>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" style="background: #ec9a29; color:white;" type="submit" id="btn_save" class="btn">Add</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="Modal_Edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="button" type="submit" id="btn_update" style="background: #ec9a29; color:white;" class="btn">Update</button>
                    </div>
                </div>
            </div>
        </div>
 ...
 $('#inputState').change(function() {
      ...
      $('#takeaway').click(function() {
                window.location = "http://139.59.11.45/index.php/HomeScreenController/order?redirect=" + "Takeaway";
            });


            function show_tables(date,time) {
                if (date) {
                $.ajax({
                    type: 'ajax',
                    url: '<?php echo site_url('Tables/tablehistory_data') ?>',
                    async: true,
                    dataType: 'json',
                        ...
                        //var len1 = length - 5;
                            ...
                            for (i = 0; i < data.length; i++) {
                                if (date === data[i].day && data[i].state === "seated") {
                                    html +=
                                        '<div class="col-md-2" style="text-align:right; margin-top:20px; display:flex; justify-content:center;">' +
                                        '<a style="text-decoration:none;" href="javascript:void(0);" class="item_edit" data-id="' + data[i].id + '" data-name="' + data[i].name + '" data-state="' + data[i].state + '" data-status="' + data[i].status + '" data-time="' + data[i].time_ + '" data-day="' + data[i].day + '"><img class="m-3 menu-items white" style="background-color:#5c965c;" src="<?php echo base_url() . 'assets/images/dinner-table-black.svg'; ?>" width="100" height="100"><h6 style="color:black;position: relative; bottom: 49px; text-align: center; ">' + data[i].name + '</h6></a>' +
                                        '</div>';
                                ...
                                if (date === data[i].day && data[i].state === "reserved") {
                                    html +=
                                        '<div class="col-md-2" style="text-align:right; margin-top:20px; display:flex; justify-content:center;">' +
                                        '<a style="text-decoration:none;" href="javascript:void(0);" class="item_edit" data-id="' + data[i].id + '" data-name="' + data[i].name + '" data-state="' + data[i].state + '" data-status="' + data[i].status + '" data-time="' + data[i].time_ + '"><img class="m-3 menu-items white" style="background-color:#608bf3;" src="<?php echo base_url() . 'assets/images/dinner-table-black.svg'; ?>" width="100" height="100"><h6 style="color:black;position: relative; bottom: 49px; text-align: center; ">' + data[i].name + '</h6></a>' +
                                        '</div>';
                                }
                            }
                            $('#show_data').html(html);
                        ...
                        //var len1 = length - 5;
                            ...
                            for (i = 0; i < data.length; i++) {
                               ...
                               var time1 = inputTime[0];
                                var compareTime = data[i].time_;
                                var time2 = inputTime[1];
                                // var time1 = JSON.stringify(inputTime[0]);
                                // var compareTime = JSON.stringify(data[i].time_);
                                // var time2 = JSON.stringify(inputTime[1]);
                                console.log("inputtime[0]",time1)
                                console.log("database",compareTime)
                                console.log("inputtime[1]",time2)
                                console.log(time1 <= compareTime &&  compareTime<time2)
                                if (time1 <= compareTime &&  compareTime<time2 && data[i].state === "seated") {
                        ...
                        var i;
                            ...
                            for (i = 0; i < data.length; i++) {
                                if (data[i].state === "seated") {
                                    ...
                                    html +=
                                        '<div class="col-md-2" style="text-align:right; margin-top:20px; display:flex; justify-content:center;">' +
                                        '<a style="text-decoration:none;" href="javascript:void(0);" class="item_edit" data-id="' + data[i].id + '" data-name="' + data[i].name + '" data-state="' + data[i].state + '" data-status="' + data[i].status + '" data-time="' + data[i].time_ + '"><img class="m-3 menu-items white" style="background-color:#f1c893;" src="<?php echo base_url() . 'assets/images/dinner-table-black.svg'; ?>" width="100" height="100"><h6 style="color:black;position: relative; bottom: 49px; text-align: center; ">' + data[i].name + '</h6></a>' +
                                        '</div>';
                                    if (date === data[i].day) {
                                        '<div class="col-md-2" style="text-align:right; margin-top:20px; display:flex; justify-content:center;">' +
                                        '<a style="text-decoration:none;" href="javascript:void(0);" class="item_edit" data-id="' + data[i].id + '" data-name="' + data[i].name + '" data-state="' + data[i].state + '" data-status="' + data[i].status + '" data-time="' + data[i].time_ + '"><img class="m-3 menu-items white" style="background-color:#f1c893;" src="<?php echo base_url() . 'assets/images/dinner-table-black.svg'; ?>" width="100" height="100"><h6 style="color:black;position: relative; bottom: 49px; text-align: center; ">' + data[i].name + '</h6></a>' +
                                    ...
                                    html +=
                                        '<div class="col-md-2" style="text-align:right; margin-top:20px; display:flex; justify-content:center;">' +
                                        '<a style="text-decoration:none;" href="javascript:void(0);" class="item_edit" data-id="' + data[i].id + '" data-name="' + data[i].name + '" data-state="' + data[i].state + '" data-status="' + data[i].status + '" data-time="' + data[i].time_ + '"><img class="m-3 menu-items white" style="background-color:#6cabc2;" src="<?php echo base_url() . 'assets/images/dinner-table-black.svg'; ?>"width="100" height="100"><h6 style="color:black;position: relative; bottom: 49px; text-align: center;">' + data[i].name + '</h6></a>' +
                                        '</div>';
                                }
            ...
            //     console.log("timeup",new Date().toLocaleTimeString())
            //     if(new Date().toLocaleTimeString()==="10:20:09 AM"){
            ...
            $('#btn_save').on('click', function() {
                ...
                var state = "empty";
                var status = "active";
                var time_ = new Date().toLocaleTimeString('en-GB');
                var today = new Date();
                var dd = String(today.getDate()).padStart(2, '0');
                var mm = String(today.getMonth() + 1).padStart(2, '0'); //January is 0!
                var yyyy = today.getFullYear();
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#Modal_Add').modal('hide');
                        if (state === "seated") {
                            window.location = "http://139.59.11.45/index.php/HomeScreenController/order?redirect=" + name;

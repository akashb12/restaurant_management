...
class ProductScreenController extends CI_Controller
{
    ...
    function fetch()
    {
        ...
        if($data->num_rows() > 0){
            foreach($data->result() as $row){
                ...
                color: black;
                cursor:pointer;  href="javascript:" onclick="edit('.$row->id.')"><h5>'.$row->name.'</h5></a>
                </div>
    ...
    function edit($id){
        ...
        $product = $this->Product_model->getProduct($id);
                $output .= '
                <form action="http://139.59.11.45/index.php/ProductScreenController/update/'.$product["name"].'" method="post" accept-charset="utf-8">
                <div class="fullSection">
                    ...
                    <div class="row">
                        ...
                        <div class=" buttons col-md-4">
                            ...
                            <button style="background-color:#A8201A;"  type="button" onclick="remove('.$id.')" class="btn btn-danger"> <span><i class="fa fa-minus" aria-hidden="true"></i></span>&nbsp;Remove Product</button>
                        </div>
                    </div>
                </div>
                </form>
	...
	function add()
     {
        if (isset($_POST['submit'])) {
                if (empty($_POST['productname']) || empty($_POST['price']) || empty($_POST['roll'])) {
                    echo "<script>window.open('index?Empty= please fill blank fields','_self')</script>";
                } else {
                $this->load->model('Product_model');
                $formArray = array();
                $cat = array();
